/****************************************************************************
 *
 * Copyright(c) 2007-2017, John Forkosh Associates, Inc. All rights reserved.
 ...
 *		mathtexlog(expression,filename) write logfile for new express
 *		usagelog(expression)  write logfile entry for all expressions
 ...
 *		ninstances(object,action,pids)#instances of object from ps ax
 ...
 * 02/17/17	J.Forkosh	Version 1.06
 * 05/16/17	J.Forkosh	Most recent change (see REVISIONDATE below)
 *
 ****************************************************************************/
...
Program ID
-------------------------------------------------------------------------- */
#define	VERSION "1.06 "			/* mathTeX version number */
#define	REVISIONDATE "16 May 2017"	/* date of most recent revision */
#define	COPYRIGHTDATE "2007-2017"	/* copyright date */
...
#endif
#if !defined(ALLPACKAGES)		/* check \usepackage{} in whitelist */
  #define ALLPACKAGES 0			/* \usepackage{} okay for all pkgs */
#endif
...
#endif
#if !defined(USAGELOG)
  #define USAGELOG "\000"		/* default usage log file */
#endif
...
#define	MAXIMAGEMETHOD 3		/* 1<=imagemethod<=MAXIMAGEMETHOD */
/* ---
 * image type info specifying 1=gif, 2=png (and 3=pbm, 4=pgm)
 * ---------------------------------------------------------- */
#if defined(GIF)
  #define IMAGETYPE 1
#endif
  ...
  #define IMAGETYPE 1
#endif
static	int  imagetype = IMAGETYPE;	/* 1=gif, 2=png (and 3=pbm, 4=pgm) */
static	char *extensions[] = { NULL,	/* image type file .extensions */
  "gif", "png",				/* 1,2=standard image types */
  "pbm", "pgm",				/* 3=portable bit map, 4=grayscale */
  NULL }; /* --- end-of-*extensions[] --- */
#define	GIFTYPE 1			/* 1=gif */
#define	PNGTYPE 2			/* 2=png */
#define	PBMTYPE 3			/* 3=pbm */
#define	PGMTYPE 4			/* 4=pgm */
#define	MAXIMAGETYPE 4			/* 1<=imagetype<=MAXIMAGETYPE */
/* ---
 ...
 * convert options
 * --------------- */
#if !defined(CONVERTOPTS)
  #define CONVERTOPTS "\000"		/* no additional convert options */
#endif
static	char convertopts[512] = CONVERTOPTS; /* options for convert */
/* ---
...
/* --- \usepackage{} whitelist (unless ALLPACKAGES=1) --- */
static	int  allpackages = ALLPACKAGES;	/* true to allow all \usepackages's */
static	char *whitepackages[] = {	/* \usepackage{} whitelist */
  #if defined(WHITEPACKAGES)		/* cc -DWHITEPACKAGES=\"filename\" */
    #include WHITEPACKAGES		/*filename with comma-separated list*/
  #endif
  "inputenc",       "amsmath",          "amsfonts",         "amssymb",
  "bm",             "calc",             "color",            "eepic",
  "epic",           "pict2e",           "preview",          "fontenc",
  "mathrsfs",       "cancel",           "ulem",             "asymptote",
  "pst-circ",       "pst-all",          "pstricks",         "pstricks-add",
  "pst-plots",      "textcomp",         "xy",               "amsthm",
  "mathtools",      "slashbox",         "extarrows",        "esint",
  "parskip",        "ifxetex",          "polyglossia",      "fontspec",
  "unicode-math",   "babel",            "lmodern",          "mathdesign",
  "microtype",      "csquotes",         "biblatex",         "hyperref",
  "ifthen",         "tikz",             "background",       "CJK",
  "CJKulem",        "CJKutf8",          "ruby",             "accents",
  "graphicx",
  NULL					/* whitepackages[] trailer */
  } ; /* --- end-of-whitepackages[] --- */
/* ---
...
#if !defined(MSGEXIT)			/* msgnumber to emit nothing and quit*/
  #define MSGEXIT 99			/* msgnumber==MSGEXIT just quits */
#endif
#if !defined(MSGNOREFERER)		/* msgnumber if http_referer==NULL */
  #define MSGNOREFERER MSGEXIT		/* default emits nothing and quits */
#endif
/* ---
 * maximum number of running instances permitted, and max# before abort
 * ----------------------------------------------------------------------- */
#if !defined(MAXINSTANCES)		/* no -DMAXINSTANCES=nnn on cc line */
  #define MAXINSTANCES 0		/* so no limit check on #instances */
#endif
#if !defined(MAXINSTWAIT)		/* no -DMAXINSTWAIT=nn on cc line */
  #define MAXINSTWAIT 10		/* wait 10 times, then quit */
#endif
#if !defined(MAXINSTHUNDREDTHS)
  #define MAXINSTHUNDREDTHS 100		/* each wait this many hundredths */
#endif
#if !defined(MAXINSTABORT)		/* abort request if #instances>ABORT */
  #define MAXINSTABORT 0		/* default 0 means no limit */
#endif
#if !defined(MAXINSTABORTMSG)		/* msg# to display if aborted */
  #define MAXINSTABORTMSG 0		/* default 0 means no message */
#endif
static	int maxinstances = MAXINSTANCES; /* overridden by \maxinstances{n} */
/* ---
 * gif for empty query_string's
 * ------------------------------- */
#if !defined(MATHTEXGIF)		/* emitted if no query_string */
  #define MATHTEXGIF "\000"		/* just quit if no MATHTEXGIF */
#endif
/* ---
 * password to bypass preceding REFERER (and MAXINSTANCES) checks
 * ----------------------------------------------------------------- */
#if !defined(PASSWORD)
...
#define	TOOMANYINSTANCES 16		/* msg# if too many instances running*/
...
/* ---
 * output file, or stdout (from shell mode)
 * ------------------------------------------ */
static	char outfile[256] = "\000";	/* output file, or empty for default*/
static	int  isstdout = 0;		/* true if \stdout directive given */
/* ---
...
#endif
#if !defined(MAXGIFSZ)
  #define MAXGIFSZ (999999)		/* max #bytes in output GIF image */
#endif
#if !defined(MAXPBMSZ)
  #define MAXPBMSZ (99999999)		/* max #bytes in output pbm/pgm */
#endif
...
int	isquery = 0,			/* true if input from QUERY_STRING */
	isqueryempty = 0;		/* empty query string */
...
struct	{ char *deny;			/* http_referer can't contain this */
	...
	#endif
	{ NULL, -999 }, { NULL, -999 },	/* dummy (avoid array subscript */
	{ NULL, -999 }, { NULL, -999 },	/*   "    warning) */
	{ NULL, -999 } };		/* trailer */
...
int	ninstances();			/* #instances mathTeX running */
int	iswait = 1;			/*false if http_referer in NOWAIT list*/
...
int	mathtexlog(), usagelog();	/* log requests */
/* --- messages --- */
...
if(  imagetype<1||  imagetype>MAXIMAGETYPE  )   imagetype=1; /*keep in bounds*/
if(imagemethod<1||imagemethod>MAXIMAGEMETHOD) imagemethod=1; /*keep in bounds*/
...
if ( !isquery )				/* empty query string */
  { char *addr = getenv("SERVER_ADDR");	/* additional getenv("") results */
    ...
    ||    remote_addr!=NULL  || http_referer!=NULL ) {
      isquery = 1;			/* set flag to signal query */
      isqueryempty = 1;			/* but it's an empty query_string */
      if ( 0 ) goto end_of_job;		/* immediately abort empty query? */
      strcpy(expression,"\\fbox{\\rm No expression supplied}"); }
  } /* --- end-of-if(!isquery) --- */
if ( isquery ) usagelog(expression);	/* immediately log queries */
/* ---
...
&&   !ispassword ) {			/* nor if user supplied \password{} */
  ...
  if ( isempty(http_referer) ) {	/* http_referer not supplied */
	...
	msgnumber = MSGNOREFERER; }	/* msgnumber for http_referer==NULL */
...
if ( !isvalidreferer ) {		/* invalid referer detected */
  if ( msgnumber == MSGEXIT )		/* this msgnumber just exits */
    goto end_of_job;			/* so just quit */
...
/* ---
 * max instances (cc -DMAXINSTANCES=n)
 * -------------------------------------- */
if ( isquery				/* have a query */
&&   iswait				/* http_referer not on NOWAIT list */
&&   maxinstances > 0 ) {		/* limit #instances running */
  char	*object = "mathtex.cgi";	/* instances of mathtex.cgi */
  int	npids=0, pids[512];		/* list of pid's running mathtex */
  npids = ninstances(object,0,pids);	/* get currently-running mathtex's */
  if ( npids > maxinstances ) {		/* too many */
    int	iwait = 0;			/* so wait till there are fewer */
    for ( iwait=0; iwait<MAXINSTWAIT; iwait++ ) { /* wait MAXINSTWAIT times */
      int ntoomany = (npids-maxinstances); /* #instances above limit */
      timewaitfor(MAXINSTWAIT*ntoomany);   /* wait time scaled by #too-many */
      npids = ninstances(object,1,pids); /* #original instances still running */
      if ( npids <= maxinstances ) break; /* enough of them have finished */
      if ( MAXINSTABORT > 0		/* check for abort after first wait */
      &&   npids > MAXINSTABORT ) {	/* so many instances, we want abort */
        if( MAXINSTABORTMSG > 0 ) {	/* msg# for abort */
          msgnumber = MAXINSTABORTMSG;	/* requested message number */
          emitembedded(msgnumber,2); }  /* emit image, suppress msgfp */
        goto end_of_job; }		/* exit with or without emitting msg */
      } /* ---end-of-for(iwait) --- */
    if ( npids > maxinstances ) {	/* quit if system still overloaded */
      msgnumber = TOOMANYINSTANCES;	/* requested message number */
      emitembedded(msgnumber,2);	/* emit image, suppress msgfp */
      goto end_of_job; }
    if ( 1 && isqueryempty ) {		/* abort empty query after wait? */
       if( !isempty(MATHTEXGIF) ) emitcache(MATHTEXGIF,5,0); /*first emit gif?*/
       goto end_of_job; }		/* then abort */
    } /* --- end-of-if(npids>maxinstances) --- */
  } /* --- end-of-if(isquery&&MAXINSTANCES>0) --- */
/* ---
 * query delay (after MAXINSTANCES delay)
 * ----------------------------------------- */
if ( isquery				/* have a query */
&&   iswait )				/* http_referer not on NOWAIT list */
...
/*--- check for explicit displaystyle/textstyle/parstyle,etc directives ---*/
...
/* --- check if shell user wants output to stdout --- */
if ( strreplace(expression,"\\stdout","",0,0) /*remove occurrences of \stdout*/
>=   1 ) { isstdout = 1;		/* found \stdout so set isstdout flag*/
  msglevel = 0;  iscaching = 0; }	/* and turn off messages, caching */
...
/* --- check for \instances{object} directive --- */
if ( (pdirective=getdirective(expression,"\\instances",1,0,1,argstring))
!=   NULL ) {				/* \instances{object} */
  int pids[512],			/* list of pid's running 'argstring' */
      npids = ninstances(argstring,0,pids); /* #currently-running objects */
  char	instring[512];			/* string replacing \instances{} */
  sprintf(instring,"\\ #instances(%s)=%d\\ ",argstring,npids);
  strchange(0,pdirective,instring);	/* replace \instances{} with result */
  iscaching = 0;			/* don't cache \instances */
  } /* --- end-of-if(getdirective("\\instances")!=NULL) --- */
...
while ( npackages < 9 ) {		/* no more than 9 extra packages */
  char	*ppackage = NULL;		/* ptr to \usepackage directive */
  ppackage=getdirective(expression,"\\usepackage",1,0,-1,packages[npackages]);
  if ( ppackage == NULL )  break;	/* no more \usepackage directives */
  else {				/* found another \usepackage */
    /* --- check \uspackage{} against package whitelist --- */
    char *package = packages[npackages]; /* user's \usepackage request */
    int	 ispkgokay = (allpackages>0?1:0); /* true if package okay */
    int  iwhite = 0;			/* whitelist index */
    if ( !allpackages )			/* check unless all packages allowed */
      for ( iwhite=0; !isempty(whitepackages[iwhite]); iwhite++ )
        if ( strcmp(package,whitepackages[iwhite]) == 0  ) { /*pkg white*/
          ispkgokay = 1;  break; }	/* set okay flag and use package */
    if ( !ispkgokay ) {
      char display[2048];		/* error message display */
      sprintf(display,			/* format error message... */
        "\\mbox{~\\underline{usepackage$\\lbrace$ %s $\\rbrace$ "
        "~not~permitted}~}", package);	/* ...with user's illegal pkg */
      strchange(0,ppackage,display);	/* and put it where \usepackage was */
      isquiet = 16;			/* bump max latex error count */
      continue; }			/* ignore illegal package request */
...
/* --- check for image type directives \gif or \png (or \pbm or \pgm) --- */
if ( strreplace(expression,"\\pgm","",0,0) /* remove occurrences of \pgm */
>=   1 ) imagetype = PGMTYPE;		/* found \pgm so set pgm imagetype */
if ( strreplace(expression,"\\pbm","",0,0) /* remove occurrences of \pbm */
>=   1 ) imagetype = PBMTYPE;		/* found \pbm so set pbm imagetype */
if ( strreplace(expression,"\\png","",0,0) /* remove occurrences of \png */
>=   1 ) imagetype = PNGTYPE;		/* found \png so set png imagetype */
if ( strreplace(expression,"\\gif","",0,0) /* remove occurrences of \gif */
>=   1 ) imagetype = GIFTYPE;		/* found \gif so set gif imagetype */
/* --- default to pdflatex and imagemethod=3 for pbm,pgm --- */
if ( imagetype==PBMTYPE || imagetype==PGMTYPE ) { /*bitmap output requested*/
  latexmethod = 2;			/* so start with a pdf file */
  imagemethod = 3; }			/* and convert .pdf --> .pbm/.pgm */
...
/* --- check for additional convert options --- */
getdirective(expression,"\\convert",1,0,1,convertopts); /*look for \convert*/
/* --- check for \cache or \nocache directive --- */
...
if ( md5hash != NULL ) {		/* md5str() almost surely succeeded*/
  ...
  if ( msglevel >= 1 )			/* check if logging */
    if ( iscaching )			/* only log images being cached */
      if ( isempty(outfile) )		/* and not saved to explicit outfile*/
        mathtexlog(hashexpr,md5hash);	/* log request for new image */
  /* ---
  ...
  ==   imagetype ) {			/* and emit cache if succeeded */
    ...
    if ( nbytes > 0 )			/* mathtex() and rewritecache() ok */
     if ( isquery || isstdout )		/* don't emit cache to shell stdout*/
...
if ( isquiet > 0 ) {			/* to continue after latex error */
    if ( isquiet > 99 ) {		/* explicit q requested */
      if ( system("echo \"q\" > reply.txt") /*reply q to latex error prompt*/
      ==   (-1) ) ; }			/* handle system() error return */
...
/* -------------------------------------------------------------------------
Construct the output path/filename.[gif,png,pbm,pgm] for the image file
-------------------------------------------------------------------------- */
...
if ( imagemethod == 2 ) {		/* dvips/convert method requested */
  ...
  /* --- add user-supplied convert options if \convert{} supplied --- */
  if ( !isempty(convertopts) ) {	/* expression has \convertopts{} */
    strcat(convertargs,convertopts);	/* add convert options */
    strcat(convertargs," "); }		/* and blank separator */
  /* ---
...
/* -------------------------------------------------------------------------
Run convert for .pdf-to-pbm/pgm
-------------------------------------------------------------------------- */
if ( imagemethod == 3 ) {		/* convert pdf --> pbm/pgm */
  /* --- init convertargs for pbm/pgm --- */
  char  convertpbm[1024] =		/* pbm/pgm -switches for convert */
	" -density %%dpi%% -trim -compress none "; /* "none" gives P1/P2 */
  strcpy(convertargs,convertpbm);	/* re-init template */
  /* --- replace %%dpi%% in convertargs template with actual density --- */
  strreplace(convertargs,"%%dpi%%",density,1,0);
  /* --- add user-supplied convert options if \convert{} supplied --- */
  if ( !isempty(convertopts) ) {	/* expression has \convert{} */
    strcat(convertargs,convertopts);	/* add convert options */
    strcat(convertargs," "); }		/* and blank separator */
  strcpy(command,makepath("",convertpath,NULL)); /*running convert program*/
  if ( isempty(command) ) {		/* no program path to convert */
...
-------------------------------------------------------------------------- */
end_of_job:
  if ( dir_stat >= 2 ) { if ( chdir("..") /*return up to working dir or home*/
                         == (-1) ) ; }	/* handle chdir() error return */
  if ( dir_stat >= 1 ) rrmdir(tempdir/*filename*/);/*rm -r temp working dir*/
  if ( isworkpath ) {			/* return from working dir to home */
    if ( chdir((!isempty(homepath)?homepath:"..")) /* best guess at home */
    ==   (-1) ) ; }			/* handle chdir() error return */
...
if ( imgmethod!=2 && imgmethod!=3 )	/* not needed for dvips/convert */
...
/* ==========================================================================
 * Function:	mathtexlog ( expression, filename )
 * Purpose:	write entry in log file -- called for new expressions
 *		that have to be rendered by latex (not called for old
 *		expressions already cached and just emitted)
 * --------------------------------------------------------------------------
 * Arguments:	expression (I)	pointer to null-terminated char string
 *				containing latex math expression
 *		filename (I)	pointer to null-terminated char string
 ...
 * ======================================================================= */
/* --- entry point --- */
int	mathtexlog ( char *expression, char *filename )
{
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
FILE	*filefp = NULL;			/* fopen(filename) for logfile */
int	nwaits=5, timewaitfor();	/* if fail, try to fopen after wait */
...
-------------------------------------------------------------------------- */
if ( !isempty(CACHELOG) ) {		/* if a cache logfile name is given */
  while ( filefp==NULL && --nwaits>=0 )	/* fopen(), and wait if it fails */
    if ( (filefp=fopen(makepath(NULL,CACHELOG,NULL),"a")) /* open logfile */
    ==   NULL )				/* failed, so wait and try again */
      timewaitfor(10);			/* wait ten-hundredths of a second */
  if ( filefp != NULL ) {		/* ignore logging if fopen() failed */
    fprintf(filefp,"%s                 %s\n", /* timestamp, md5 file */
    ...
    if ( 1 ) {				/* display remote_ip address */
      char ipaddr[999],			/* remote_ip address line */
           *remp = ipaddr;		/* ptr within line */
      strcpy(ipaddr,"ipaddr:");		/* start line with stub */
      if ( !isempty(remote_addr) )	/* show remote_ip addr if we have one*/
        strncat(ipaddr,remote_addr,256); /* concat remote_ip address */
      else strcat(ipaddr,"xxx.xxx.xxx.xxx"); /* or dummy */
      while ( 1 ) {			/* printed in parts if needed*/
	fprintf(filefp,"%.*s\n",loglen,remp); /* print a part */
	if ( strlen(remp) <= loglen ) break;  /* no more parts */
	remp += loglen; }		/* bump ptr to next part */
      } /* --- end-of-if(1(display remote_addr) --- */
    fprintf(filefp,"%s\n",dashes);	/* separator line */
    fclose(filefp);			/* close logfile immediately */
    } /* --- end-of-if(filefp!=NULL) --- */
  } /* --- end-of-if(!isempty(CACHELOG)) --- */
return ( 1 );
} /* --- end-of-function mathtexlog() --- */


/* ==========================================================================
 * Function:	usagelog ( expression )
 * Purpose:	write entry in log file -- called for all expressions
 * --------------------------------------------------------------------------
 * Arguments:	expression (I)	pointer to null-terminated char string
 *				containing latex math expression
 * --------------------------------------------------------------------------
...
char	*http_referer = getenv("HTTP_REFERER"), /* referer using mathTeX */
	*remote_addr  = getenv("REMOTE_ADDR");  /* remote ip using mathTeX */
/* -------------------------------------------------------------------------
write message to usage log file
-------------------------------------------------------------------------- */
if ( !isempty(USAGELOG) ) {		/* if a usage logfile name is given */
  while ( filefp==NULL && --nwaits>=0 )	/* fopen(), and wait if it fails */
    if ( (filefp=fopen(makepath(NULL,USAGELOG,NULL),"a")) /* open logfile */
    ==   NULL )				/* failed, so wait and try again */
      timewaitfor(10);			/* wait ten-hundredths of a second */
  if ( filefp != NULL ) {		/* ignore logging if fopen() failed */
    fprintf(filefp,"%s  %s  %.50s\n",	/* timestamp, ip_addr, referer */
    timestamp(TZDELTA,0),		/* timestamp */
    (!isempty(remote_addr)?remote_addr:"xxx.xxx.xxx"),
    (!isempty(http_referer)?http_referer:"http://xxx") );
    fprintf(filefp,"   %.75s\n",expression); /* expression */
    fclose(filefp);			/* close logfile immediately */
    } /* --- end-of-if(filefp!=NULL) --- */
  } /* --- end-of-if(!isempty(USAGELOG)) --- */
return ( 1 );
} /* --- end-of-function usagelog() --- */
...
&& !isstdout ) {			/* and not output to shell stdout */
  /* --- emit mime content-type line --- */
  fprintf( emitptr, "Cache-Control: max-age=%d\n",maxage );
  fprintf( emitptr, "Content-Length: %d\n",nbytes );
  if ( imagetype==GIFTYPE || imagetype==PNGTYPE ) /* .gif or .png file */
    fprintf( emitptr, "Content-type: image/%s\n\n",extensions[imagetype] );
  else					/* .pbm or .pgm text file */
    fprintf( emitptr, "Content-type: text/plain\n\n" );
  } /* --- end-of-if(isdepth==0&&maxage>=0&&!isstdout) --- */
/* -------------------------------------------------------------------------
set stdout to binary mode (for Windows)
-------------------------------------------------------------------------- */
#if defined(WINDOWS)				/* compiled under Windows */
  if ( imagetype==GIFTYPE || imagetype==PNGTYPE ) { /*only for gif,png files*/
    /* emitptr = fdopen(STDOUT_FILENO,"wb"); */  /* doesn't work portably */
      ...
      #endif
    #endif
    } /* --- end-of-if(imagetype==GIFTYPE||imagetype==PNGTYPE) --- */
#endif
/* -------------------------------------------------------------------------
...
Allocations and Declarations
-------------------------------------------------------------------------- */
int	isbinary = (imagetype==GIFTYPE||imagetype==PNGTYPE); /* binary read */
char	*mode = (isbinary?"rb":"r");	/* binary gif/png, text pbm/pgm */
FILE	*cacheptr = fopen(cachefile,mode); /*open cachefile for (binary) read*/
...
int	buflen = 256,			/* #bytes we try to read from file */
	...
	isgif = (imagetype==GIFTYPE||imagetype==PNGTYPE), /* gif or png */
	maxbytes = (isgif?MAXGIFSZ:MAXPBMSZ), /*max#bytes returned in buffer*/
...
/* ==========================================================================
 * Function:	ninstances ( char *object, int action, int *pids )
 * Purpose:	parse output of "ps -ax" for instances of object
 * --------------------------------------------------------------------------
 * Arguments:	object (I)	char * to null-terminated string
 *				containing object to search "ps -ax" for
 *		action (I)	int containing 0 to return (-1)-terminated
 *				list of found object pid's, or...
 *				+1 returns #instances matching already-found
 *				pids; -1 returns #instances not matching any
 *		pids (I/O)	int * returning pid's of found objects
 *				if action=0, or containing (-1)-terminated
 *				list of previously found pid's if action=+-1.
 * --------------------------------------------------------------------------
 * Returns:	( int )		#instances of object found in "ps -ax" output,
 *				or -1 for any error.
 * --------------------------------------------------------------------------
 * Notes:
 * ======================================================================= */
/* --- entry point --- */
int     ninstances ( char *object, int action, int *pids )
{
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
int     nfound = (-1),			/* #instances of object found */
	maxfound = 255;			/* quit after maxfound instances */
char    command[256];			/* ps -ax */
FILE    *out = NULL;			/* ps -ax's stdout */
char    line[256];			/* line from ps -ax's stdout */
int	ipid=0, npids=(-1);		/* pids[] index, #pids when action=+-1*/
/* -------------------------------------------------------------------------
search ps -ax's output for instances of object
-------------------------------------------------------------------------- */
/* --- initialization --- */
if ( object==NULL ) goto end_of_job;	/* no input supplied */
/* --- command --- */
sprintf(command,"ps ax");		/*construct cmd*/
/* --- use popen() to invoke command --- */
out = popen( command, "r" );		/* issue and capture stdout */
if( out == NULL ) goto end_of_job;	/* failed */
/* --- read the pipe one line at a time --- */
nfound = 0;				/* init count */
while ( fgets(line,255,out)		/* get next line */
!= NULL ) {				/* not at eof yet */
  if ( strstr(line,object) == NULL ) continue; /* not an instance of object */
  if ( pids != NULL ) {			/* caller supplied pids[] list */
    int thispid = atoi(line);		/* initial segment of line is pid */
    if ( action == 0 )			/* caller wants all pid's returned */
      pids[nfound] = thispid;		/* store thispid in current pids[] */
    if ( action != 0 ) {		/* check this pid against list */
      if ( npids < 0 )			/* determine #pids[] on 1st pass */
        for ( npids=0; npids<maxfound ;npids++ ) /*look for terminating (-1)*/
          if ( pids[npids] < 0 ) break;	/* break with index of (-1) pids[] */
      for ( ipid=0; ; ipid++ ) { 	/* loop through existing list */
        if ( pids[ipid] < 0 ) break;	/* break at end-of-list */
        if ( thispid == pids[ipid] ) break; } /* or break if match found */
      if ( action > 0 /*==(+1)*/ )	/* only count instances in pids[] */
        if ( thispid != pids[ipid] ) continue; /* don't count this instance */
      if ( action < 0 /*==(-1)*/ )	/* only count instances NOT in pids[]*/
        if ( thispid == pids[ipid] ) continue; /* don't count this instance */
      pids[npids+1+nfound] = thispid;	/* save new pids after end of list */
      } /* --- end-of-if(action!=0) --- */
    } /* --- end-of-if(pids!=NULL) --- */
  nfound++;				/* count another instance of object */
  if ( nfound >= maxfound ) break;	/* quit after maxfound instances */
  } /* --- end-of-while(fgets()!=NULL) -- */
if ( pids != NULL ) {			/* caller supplied pids[] list */
  if ( action != 0 )			/* shift left new pids[] after -1 */
   if ( action<(-1) || action>(+1) )	/* only if signalled by <-1 || >+1*/
    for ( ipid=0; ipid<nfound; ipid++ )	/* new pids to beginning of list */
      pids[ipid] = pids[npids+1+ipid];
  pids[nfound] = (-1);			/* (-1)-terminate pids[] list */
  } /* --- end-of-if(pids!=NULL) --- */
/* --- pclose() waits for command to terminate --- */
pclose( out );				/* finish */
end_of_job:				/* back to caller */
  return ( nfound );			/* with #instances found */
} /* --- end-of-function ninstances() --- */
...
static	unsigned char *image15[] = {
  ...
  NULL } ; /* --- end-of-image15[] --- */
/* ---
 * image16[] contains an 1840-byte gif rendering of expression
 * \parstyle \usepackage{color}\color{red}\fparbox{(16) Too many
 * instances of mathTeX running\\Please try again later.}
 * ---------------------------------------------------------- */
#define	IMAGETYPE16 1			/* 1=gif, 2=png */
#define	IMAGESIZE16 1840		/* #bytes in image16[] */
static	unsigned char *image16[] = {
  (unsigned char *) "GIF89aN\x1%\x0\xb3\x0\x0\xff\xff\xff\xff\x0\x0\xff\x92" 
  "\x92\xff\xd6\xd6\xff\x18\x18\xff\xe\xe\xff\xb0\xb0\xff\x4\x4\xff\b\b\xff" 
  "\\\\\xfftt\xff$$\xff\x34\x34\xff\x46\x46\x0\x0\x0\x0\x0\x0!\xf9\x4\x1\x0" 
  "\x0\x0\x0,\x0\x0\x0\x0N\x1%\x0\x0\x4\xfe\x10H \xc4\xbc\x18\xa7\xcc\xbb" 
  "\xff`(\x8e\x64i\x9eh\xaa\xael\xebN\x3q\n\xcck\xdfx\xae\xef\x98\x61\xf1@`" 
  "apQ 2\x8c\x64\x63\xb2\x30x\f\x81\x3\x81\x10\bL\xf\x1\x62P\xb7\x38l\xbf" 
  "\x19\x1\x61\xb1\xe0)\xb0\x19\xa8\x94j%`\xb5\x17q\xf5@lT\x11\?0\xa0\xbb" 
  "\x12O\x14S\x1\x18\b\n\x14Y\x12\x6\x5\x1e~\x12\aG\x12\x4Nz6\r\x8a(5\x93&Y" 
  "\ne7\x98\x12\x2\x82\x18\x8c\x0\x8e\x13\x91\x1d\t\x1\x1b\x12\x5y\x93\x95+" 
  "\x4\xaf\x0\xaa\x17\xb1\x0\x3\x9d\x12\x8f\x1c\ny\xa6\xa0\xb4\x99\xc4\x0" 
  "\xbd\xc5 P;\xc7\xca\x18\xbf\x13\xc1\x14\xc3\x13\x5\x87\x2K\xc8,\xa2\x17" 
  "\xb6\x13\x1\xd8\x19\v\x85\x1c\tp\xd1\x3\xac\xd9\x93\xba\xdb\xea\x1d\xcd" 
  "\x37\xec\x17\xf0\x13\xe5\xd0\xbd\xe8\x1e\x3V\x0\x32&>\x12\x6\x18\x90\x94" 
  "\xc9\x80\x17\r\xdb\xf6\x35`\xd0`\x1\x9cZ\xfe@D\x9b\xc0\xb0!\x9c\x8a\xe}U" 
  "1\xd0 V\x82\x6\xfe\x4\xc0ql\xc8J\xc1\xc6\x85\x96\xec|\x3\x0%\x0-\x1h\x0" 
  "\x98\f\xc0\x91\x81%\x0\f>2(S)\x0\x19\'#\x17\x94<i\xf3\xc2\xce\x9dZ\x16," 
  "\xfc\x94s\xe1.$\x1d\x33\x2\xf2\xf9\xe9\xcfI\x8f EFe\xd5\xf3\'K\x9a(9L" 
  "\x94\xa0\x34\t\x6;\x5\b\x92\v@h\x81\"\x1\bD1\xe0\x37\xb3\xa6\xa5\xba\x61" 
  ")\xc4\xc4[4 \x81\x9d\x1d\x1f\xc1\xcd\xd0\xcdP\xa7\x33\x45n~\x18K\x80U" 
  "\xb3\xc6\x88\xda\x15Y\xc5+m\xae\x43\xc6:\xb9\x94`\x92\x15\x2l\xa1(>\x9c" 
  "\x10\x9a\x33\x65\x63K\x14`\x8a\x61\xfa\x2\x2\xcd\?:",
  (unsigned char *) "\xf3\xc2\x36\x5\x92\f\x4\xac\x42\xca\\\x1d\x11\x3\xe4" 
  "\xaf\x91=\xc8\x36\x66y\x9f\x96\xd7\x12\x36\xcbl\a\xc5\x33\xb8{\x83r\?/%y" 
  "\xed\xaf\x83&O\xf9\x1b\xfe\xd9\xb4sP\xdb\xb8\x63;\xc0j\x93\x84\x4\x8a" 
  "\xcf\x87\?\r \xc0\x38\xbd\"&\x96\xe6\xc5`\xbe\xb1\xaa\x13\xa0h!\xd0\x8b" 
  "\xe6\xf9q\x5t\xfe\xa2\x1f$\xbb\xac\x4\xc0t\xf9m3`\?e$p\x80$\xe3\x64W\xf" 
  "\x80\x2\x62\x36\xc6W\x92\x34\x90\xc0|\xe\xd6\xf2 g\x1c\xd8\x87@\r\xf4" 
  "\xf4`a\x7f\x92$@ak\t\xee\xf7T#\xc7px\x10\x35\x1\xe0\x17\x86\x64\xf0\xd4" 
  "\xf6\x95\x8b\x91\xf1\b\\\x8f\x4&7\xe{\xe8\x11\xa6\xa0r\xed\xed\x2S|\xc7" 
  "\xd8q\x1\x7fN\x9erL\x82\xday\x93\x87\x1I\x1cPa\x95\x12\x34\xe0\x85\x2j" 
  "\xcds$\x97q\xf9\x64U\xfYni[-\x92\xcd\xd5\xd1\x42\xfe\x94\xf9\xe2\x4QBrD" 
  "\x89\x62r\xd9\xde\x95i\xb2\x18\xdc\x9a\x18L\xe4\x66G\f\xf4\xd6\x80\x0\v" 
  "\xf8\xb7\b\x8e\xdb\xe8\x98\xa3\?\x8f\xfe\xf9\x63\?\xfe\\\xb8 \x5\xe9\xb1" 
  "y\x81{\xde|\xa2@\xa6\x62\x1d\x33\xd7\x93#\xb6\xc3\x1f\a\x91&\xf7\xc3\x2" 
  "\b\x10q\xe1\xa4:^\x86\xe5\x13\x63\x2J\xc1\\w\x89\x62\x1\xab\xae\xaaI\xa9" 
  "\xa6\x46U\a\n\xae\x1c\x8c*\xe5\xa4i4\x1a\x91r\xbc\xfe\x32\xc8\xa2\x5\xa9" 
  "^ \xa8\xb0\xb9h6\xe5(\x8cr\x99\xea\xb6\xb5\xfez\xe0Rj\x19\x64\xa4k\xe0pz" 
  "^o\x1e\xc8\xd7\xcek\"\xce\t\xeb\xb2\x16\x18\xd7J\x13\x3){A\x1\x5\xa4\x93" 
  ",\x95k2@\x90(\x12\x36 /\x0\x1\xe\x14)\x14\x4\xf9\x80\x19K8\xfd\x1b\xe2" 
  "\xba\x65\xe0\xc9\xaf\xb7{^\xa6\x45\xc1\x6\x4\xfcn\x6\xea\xc2\x41\x10\x1Z" 
  "\xa8\xa2/\xb6&R\x3\xa9\xbd\x1bO\x9a\xaa\xbb\xc9\x8d\xfb\xc7#\x1d\x32\x31" 
  "r\ac\x15\x80\xcd\x80\x36\x3y\xa3\x9e\x9b-\x88\x0\x1`\xce\x17\xab!\x1f" 
  "\xc0#\xb4\f",
  (unsigned char *) ";Q\x13\x90 \x3\x80\x89\xd9\xcf\x41\xa3\x1c\xeb\x12\x1" 
  "\x6\xb4\x44\xd2\xadt\x90\xf3\x8e\xc8\x92lk\xcfO\x3m\xc0>\xb9\x14r4\xc7SV" 
  "\x9d\v6\v\xb9\xb6\xb0\x33\xd5\x1d\x32\x0\x2\x82\xa1|\xf6\xa4w\x3=\x1a$" 
  "\x4!ZE\x1\x9f$0\x6\xc8\xae\xed}\x81.\xd5\x4\x10 \x1c\x64\x14\xca\xf8\x2" 
  "\x8e\xfb\x12WZs\xf9\x94H\x14\xfe\x85\x14\xda\x91\x41\f\x18\xa1\x38\x96UT" 
  "\xc5\xb2L\x93\x93\xfey\xe5\f)\xd1\x4\x45\x80K\xa0y\x3\x9c{Ny\xe8\xae\xff" 
  "\xf5I\xe3\x98,\xd5P\x98\x17\x34N8\\\x8a\xbf\xf7\xf2\xe9U4Q\xcd\x1\x99" 
  "\x87\x4\xfb\x1\x98\xd8T\x83\xec\xa0\xd7\x18\x90[\x7fgD\x16\xe4\x35$P\xe6" 
  "\xf\xaaT\x81\xaeLX\x14 |\x2Jq\xc2\x16\xf4\xa8\xa3\x1f\xba\xec\xa6\xcf." 
  "\xfdL\x7f\xc3\xc1I\ns\xbb\x83\x3\x98\xf6\xe7\xaf\xff\xfe,\b0\x4\"nqM\n" 
  "\x16\x30\r\xfe\x8d\xa0\x36\x36\x32\xa0\x2\x17\xa8\xc0\x86\xc4\x41\x31" 
  "\x88:\x81\x1\x46\xc7\xc0\x45$\xc1p\x15\xcc\xa0\x6\x89\xb1\x10\x42\xd9" 
  "\xe8\x19%H\xe0\x6GH\xc2\x12\x9a\xf0\x84(L\xa1\nW\xc8\xc2\x16\xba\xf0\x85" 
  "\x30\x8c\xa1\fgH\xc3\x34\x14\xb0\x86,\x0\b\xew\xa8\xe\xffy\xcf-\xdd\xd1" 
  "\v\xb5v\xc8\a\x13\xc0\x64\x88<L\xe2\x16\xe8\x6\x3~\x10\r\f\"\xcc\x4.\x8c" 
  "\x88\x44\xd7)\xf1\x8a\x37`\"\xfe\x45\xfc#\xb1\x65\xb0\xb0\x8b\x2\xc4\xa2" 
  "\x18Y\xa0\xc5.q\n\x8c\xf1H\xd4\x17\x91(\x8f\x31\xba\x11\x5\x65\xc4\xc9!" 
  "\xe8Q\x16L\x4%\x1dM\xc1\xda\x1e\x96\xc2\x81\xae\x34\x41\x15mQ\x84J\x96" 
  "\xd0\x92\x61\xdc\x11\x6\x7fQJ\x3\xee\xb4\x15P\xec\x85(\xa0\x9a\xd8!\xfT" 
  "\r\xaf\xec\xd1,\x80\xe4\x44$\xdf\x98\xc4\x32\x42\xe1\x66\xdb\xc0\r$\x96" 
  "\x80\x9c\x8a\xa9\xc6/\xbc\x90\x8e\x46\xb8\xe1\x9e%=\x11\'\x86+\xa5r\xc8" 
  "\x93\x9c\xa6\x65\x46U\xe0\xb1\xcaw\xde\x11J\xd8\xf8\xc9\x18\xaaT\xc5u",
  (unsigned char *) "8\xf9\xc6\xb8$a\x1\xb3\xe0\x97\x8cj\xb1\"\aA\xe8\x95" 
  "\x31\x83[\x1c\xe2\xc6\xb6T\xac\xa8=Cb\x85\x8a\xe6%\xa9\x5\xbd\nU\xdb\xd8" 
  "&\xc1\xe\xb3\x8d\x65\xda\x87\x98\x63\x44\xc0\x8cJ&\xc7\x37\x15\n\x11}2F" 
  "\xf1\\\xf7\rB}o9\xd3\xcc\x80\x97\x64\xc2\xbb\xfc\xc4\xd3R\v\xc3\x12\x3" 
  "\xb4\xd4M\x94\x81s\x1e\xf1\x94P;\xed\xf9Jt\xa6s\x9dy\x92\x63\x38\xe3Z" 
  "\xe5\xac\x61)N\xa2 \x90\x90\x5\xce)\xab(6\xebU\x1e$\xc8G}\x15\xad}\x91" 
  "\x85\xa2\xaf\xd2\xa8\xb1\xf2\xe9P7\xaa\x93\x97\xe0\xf1\xd8\xc0\n\xb6\xba" 
  "\x96\x85\xc2\x63\xab<PC\xa9\x91\xaf\xe\xcc\x94^s\xfa\xa9\xc1\xba\x35\x34" 
  "v\n5c\xa2\x38T@w\xdaR%\xbe\xf4\xa0\xdc\\\x9b\xcf\x80\x46\x6\xa5\x8d\xd3j" 
  "\x1c \x9b-\x15\xcaRp\x1eGlz\x13\xd3W\xc1\x64\x34\x83\x9at\xaa`\xd2j!\xd4" 
  "\x36\x80\xd4H\x6\x31M\xa5\xa1\x1c\xac@\v\xe8]\xcf,8Q\x1e\xe7tW\xd3=`\xcf" 
  "\x3\xce\x3_\xf0\xc2\xf1\x81\xd7qn9UP\x1c\x11\f\xcb<\xf6\xa9Oz\x19\xb0+c" 
  "\x9b\xd7\xba\xbb>/|\xef)L\\7\x8b\f\xfc\x95\xc0\x7fZ\x98\xe0&9K\xda\xa6\"" 
  "\xf0\x4\xe$\xcdhK\xcbZN\xd2\x80\x1\x18\x4\x41\a\xf9\xd8\xda\xda\xf2 \x2" 
  "\x0;",
  NULL } ; /* --- end-of-image16[] --- */
/* --------------------------------------------------------------------------
...
static	unsigned char **imagetable[] = { NULL,
  ...
  image13,    image14,    image15,    image16,    NULL };
int	imagesizes[] = { 0,
  ...
  IMAGESIZE13,IMAGESIZE14,IMAGESIZE15,IMAGESIZE16,0 };
int	imagetypes[] = { 0,
  ...
  IMAGETYPE13,IMAGETYPE14,IMAGETYPE15,IMAGETYPE16,0 };
/* ---
 * other variables
 * --------------- */
